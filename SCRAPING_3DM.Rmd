...
game_df_full %>% 
  select(DC:`足球`) %>% 
  princomp() -> label_pca1
summary(label_pca1) #第一行标准差平方为解释了多少方差，第二行为解释的方差占方差总和的比例，第三行为累计解释的方差，取前38个主成分累计解释方差超过80%
label_pca1$loadings #每个变量的负载，只保留最重要的那些
label_pca$scores[1:10, 1:8]  #各样本在不同主成分上的负载，用于回归
plot(label_pca, type = "lines", main = "Screeplot")
```
...
game_df_full %>% 
  select(DC:`足球`) %>% 
  prcomp() -> label_pca2
summary(label_pca2) #第一行标准差平方为解释了多少方差，第二行为解释的方差占方差总和的比例，第三行为累计解释的方差，取前38个主成分累计解释方差超过80%
